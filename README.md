...
☁️ **原子级轻量**：内存占用不到**30MB**，单SQLite文件存储架构  
...
9. **为什么添加后的Connect只显示了一部分？**  
   因为后端会尝试获取所有connect的实例信息，如果某个实例挂了或者无法访问则会被抛弃，只返回获取到的有效connect实例的信息给前端。

10. **为什么无法在ARM架构的设备上运行？**  
   由于本人技术较菜加上精力有限，暂时无法解决go-sqlite和cgo的跨平台问题，因此当前只提供了x86平台的Docker部署。
