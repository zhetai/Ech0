<template>
  <div
    class="bg-white rounded-md ring-1 ring-gray-200 ring-inset mx-auto shadow-xs hover:shadow-sm"
  >
    <div class="mx-auto w-full px-3 py-4">
      <!-- The Title && Nav -->
      <TheTitleAndNav />

      <div class="mt-6 px-3 gap-2 font-serif flex justify-between items-baseline">
        <div class="text-[#5d7183] text-md">
          <span>欢迎光临! <RouterLink to="/auth" class="font-bold">请登录</RouterLink></span>
        </div>
        <div class="text-gray-300 text-xs italic">
          Powered by <a href="https://github.com/lin-snow/Ech0">Ech0</a>
        </div>
      </div>
    </div>
  </div>
</template>

<script setup lang="ts">
import TheTitleAndNav from './TheEditor/TheTitleAndNav.vue'
</script>
