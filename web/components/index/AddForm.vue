<template>
  <div class="mx-auto mt-2 max-w-sm flex justify-end text-gray-400">
  ...
  <UCard class="mx-auto mt-2 max-w-sm">
    ...
    <div class="">
      <UTextarea v-model="MessageContent" size="sm" color="gray" placeholder="一吐为快！" autoresize class="mb-4" />
      <div class="flex justify-between items-center">
        <div class="flex items-center justify-start gap-2">
          <!-- <UInput size="sm" color="gray" :trailing="true" placeholder="你の名字" v-model="Username" class="w-24" /> -->
          <input id="file-input" ref="fileInput" type="file" accept="image/*" @change="addImage" class="hidden"
            placeholder="选择图片" />
          <!-- 使用 NuxtUI 的 UButton 触发文件选择 -->
          <UButton color="gray" variant="solid" class="cursor-pointer" size="sm" icon="i-fluent-image-20-regular"
            @click="triggerFileInput" />
        </div>

        <div class="flex gap-2">
          <!-- 清空表单 -->
          <UButton icon="i-fluent-broom-16-regular" variant="solid" color="gray" size="sm" @click="clearForm" />

          <!-- 添加Ech0 -->
          <UButton icon="i-fluent-add-12-filled" variant="solid" color="gray" size="sm" @click="addMessage" />
        </div>
      </div>
    </div>

    <!-- 显示图片 -->
    <div v-if="ImageUrl" class="w-5/6 h-auto shadow-md rounded-md mx-auto mt-5 overflow-hidden">
